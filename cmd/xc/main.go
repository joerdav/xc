...
import (
	...
	"github.com/joerdav/xc/parser/md_parser"
...
func searchUpForFile(curr string, heading *string) (models.Tasks, string, error) {
	...
	if err != nil && !errors.Is(err, fs.ErrNotExist) && !errors.Is(err, md_parser.ErrNoTasksHeading) {
		return nil, "", err
	}
...
func tryParse(path string, heading *string) (models.Tasks, string, error) {
	...
	p, err := md_parser.NewParser(b, heading)
	if err != nil {
